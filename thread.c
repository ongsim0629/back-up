...
/* Updates the thread's effective priority based on donations. */
void
update_priority(struct thread *t) {
    t->priority = t->original_priority;
    if (!list_empty(&t->donations)) {
        struct thread *highest_donor = list_entry(list_front(&t->donations), struct thread, d_elem);
...
thread_update_priority(struct thread *t) {
    ...
    update_priority(t);
    /* If the thread is in the ready list, remove and re-insert it to maintain order. */
...
thread_set_priority(int new_priority) {
    ...
    if (!list_empty(&ready_list)) {
        ...
        if (highest_ready->priority > curr->priority) {
            thread_yield();
        }
    }
    intr_set_level(old_level);
